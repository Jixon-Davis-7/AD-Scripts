...
$authuser = Read-Host "Enter DL samaccount name which has to be added as authorized sender:"


$userToAdd = (Get-ADUser -Identity $authuser -Properties distinguishedname).distinguishedname
#$userToAdd = "CN=Authorized PBP Communication,OU=Distribution,OU=Groups,OU=Objects,DC=ads,DC=autodesk,DC=com"

try
{
 # Retrieve the current dlMemSubmitPerms value
 $dl = Get-ADGroup -Identity $distributionList -Properties authOrig
 $currentauthOrig = $dl.authOrig

 # Check if dlMemSubmitPerms is $null and initialize it as an empty array if needed
 if ($null -eq $currentauthOrig) {
     $currentauthOrig = @()
 }

 # Add the new group to dlMemSubmitPerms
 $updatedauthOrig = $currentauthOrig + $userToAdd

 # Update the distribution list with the new dlMemSubmitPerms value
 Set-ADGroup -Identity $distributionList -Replace @{ authOrig = $updatedauthOrig } -Credential $credAdmin -ErrorAction Stop
