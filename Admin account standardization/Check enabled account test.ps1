...
$excelPath = "C:\Users\davisj1\VSCode\My_Scripts\Powershell\Admin account standardization\Output.xlsx"
...
if (Test-Path $excelPath) {
    ...
    Write-Host "File not found: $Outputfilepath"
}
...
#To get OU of user account
$dn = $Madminuser.DistinguishedName
$ouOnly = ($dn -split ',')[1..($dn.Length)]
...
# Separate users into enabled and disabled lists
$enabledUsers = $Userdetails | Where-Object { $_.Enabled -eq $true }
$disabledUsers = $Userdetails | Where-Object { $_.Enabled -eq $false }

# Export each to a separate sheet in the same Excel file

$enabledUsers | Export-Excel -Path $excelPath -WorksheetName "EnabledAccounts" -AutoSize -ClearSheet
$disabledUsers | Export-Excel -Path $excelPath -WorksheetName "DisabledAccounts" -AutoSize
